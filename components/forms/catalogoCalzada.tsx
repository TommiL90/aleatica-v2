...
interface FormValues {
  id: string
  nombreCalzada: string
  codigoCalzada: string
}
...
const SchemaForm = Yup.object().shape({
  ...
  nombreCalzada: Yup.string()
    .trim()
    .min(2, 'Demasiado corto!')
    .max(80, 'Demasiado largo!')
    .required('Requerido'),
  codigoCalzada: Yup.string()
    .trim()
    .min(2, 'Demasiado corto!')
    .max(80, 'Demasiado largo!')
    .required('Requerido'),
...
function CatalogoCalzadaForm(props: FormProps) {
      ...
      >
        {({
          ...
          <Form>
            <div className="mb-6">
              <label
                htmlFor="nombreCalzada"
                className="mb-2 block text-sm font-medium text-gray-900 dark:text-white"
              >
                Agregue nombre de calzada
              </label>
              <Field
                disabled={disabled}
                name="nombreCalzada"
                className={cn(
                  ...
                  errors.nombreCalzada && touched.nombreCalzada
            ...
            <div className="mb-6 grid gap-6 md:grid-cols-2">
              <div className="mb-6">
                <label
                  htmlFor="codigoCalzada"
                  className="mb-2 block text-sm font-medium text-gray-900 dark:text-white"
                >
                  Agregue codigo de calzada
                </label>
                <Field
                  disabled={disabled}
                  name="codigoCalzada"
                  className={cn(
                    'block w-full rounded-lg border p-2.5 text-sm',
                    errors.codigoCalzada && touched.codigoCalzada
                      ? 'border-red-400 bg-red-100 text-red-800 focus:border-red-400 focus:ring-red-400 dark:border-red-600 dark:bg-red-700 dark:text-red-400 dark:placeholder-red-400 dark:focus:border-red-500 dark:focus:ring-red-500'
                      : 'block w-full rounded-lg border border-gray-300 p-2.5 text-sm text-gray-900  focus:border-blue-500 focus:ring-blue-500 focus-visible:border-blue-500 dark:focus:border-blue-500 dark:focus:ring-blue-500',
                  )}
                />
...
export default CatalogoCalzadaForm
