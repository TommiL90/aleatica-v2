...
import {
  applySafetyDefenseChanges,
  getSafetyDefenseColumns,
  getSafetyDefenseEmpty,
  getSafetyDefenseMediciones,
  getSafetyDefenseRows,
...
const SafetyDefenseMeasurement = ({
  specialty,
  ...
  const [columns, setColumns] = useState<Column[]>(getSafetyDefenseColumns())
  ...
  const [item, setItem] = useState<Medicion>(getSafetyDefenseEmpty)
  ...
  const handleChange = useCallback(
    (changes: CellChange<any>[]) => {
      setMediciones((prevUnidades) =>
        applySafetyDefenseChanges(changes, prevUnidades, getSafetyDefenseEmpty),
      )
    },
    [setMediciones],
  )
  ...
  useEffect(() => {
    try {
      if (data) {
        const updatedUnidadesSimples = moreSafetyDefenseRows(
          data.result.map(processUnidad),
          25,
        )
        setMediciones(updatedUnidadesSimples)
      } else {
        setMediciones(moreSafetyDefenseRows(getSafetyDefenseMediciones(), 25))
      }
    } finally {
    }
  }, [data, processUnidad])
      ...
      <div style={{ margin: '0 20px' }}>
        <SpreadSheet.Root>
          ...
          <SpreadSheet.Body
            ...
            rows={rows}
            columns={columns}
            emptyElement={getSafetyDefenseEmpty(mediciones.length + 1)}
...
export default SafetyDefenseMeasurement
