...
import IndicadorZonaGeografica from '@/components/forms/indicadorZonaGeografica'
...
const creator = async (
  url: string,
  { arg }: { arg: { name: string; mtCountryId: string } },
) => {
  return fetch(url, {
    method: 'POST',
    headers: {
...
export default function ModalNewItem(props: PropsModalNewItem) {
  ...
  const { trigger, error } = useSWRMutation(
    `${process.env.API_URL}/MtGeographicalArea/Create`,
    creator /* options */,
  )
  const {
    data: dataFindById,
    ...
    props.itemSelected > 0
      ? `${process.env.API_URL}/MtGeographicalArea/FindById/${props.itemSelected}`
      : null,
    fetcher,
  )
  const updateMutation = useSWRMutation(
    dataFindById !== undefined && dataFindById.status == 200
      ? `${process.env.API_URL}/MtGeographicalArea/Update/${dataFindById.result.id}`
      : null,
    updater /* options */,
  )
  ...
  const saveAdministracion = async (values: any): Promise<any> => {
    let toastId
    try {
