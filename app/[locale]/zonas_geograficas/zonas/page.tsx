...
export default function ZonasGeograficas() {
  ...
  const { data, mutate, isLoading } = useSWR(
    `${process.env.API_URL}/MtGeographicalArea/GetAllPaginated?Page=${pageIndex}&PageSize=${pageSize}&SearchByProp=Name&SearchCriteria=${searchInput}&MtCountryId=${filtroPais}`,
    fetcher,
  )
  const mutation = useSWRMutation(
    `${process.env.API_URL}/MtGeographicalArea/DeleteAll`,
    deleter,
  )
    ...
    <main>
      ...
      <section className="max-h-screen w-full bg-white dark:bg-gray-900 lg:m-auto lg:w-10/12">
        ...
        <div className="mx-auto max-w-screen-xl px-4 pb-8 pt-8">
          <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
            <Table
              titulo="Zonas geograficas"
              descripcion="Repositorio de zonas geograficas para las unidades de negocio."
              ...
              searchInputPlaceholder="Buscar zona"
              filterText="Filtros"
              columsValues={
      ...
      {modalDetail ? (
        <ModalDetail
          ...
          title="Detalles de zona geografica"
          itemSelected={itemId}
      ...
      {modalDelete ? (
        <ModalDeleteRow
          titulo={`Eliminar ${selectedRows.length > 1 ? 'zonas' : 'zona'}`}
          mensaje={`¿Estás seguro de que deseas eliminar ${selectedRows.length > 1 ? 'estas zonas' : 'esta zona'} de la lista? Una vez ${selectedRows.length > 1 ? 'eliminadas' : 'eliminada'}, no podrás recuperar los datos asociados. `}
