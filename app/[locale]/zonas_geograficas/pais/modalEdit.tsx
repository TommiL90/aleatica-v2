...
import Indicadoryear from '@/components/forms/indicadorYear'
...
export default function ModalNewItem(props: PropsModalNewItem) {
  const { trigger, error } = useSWRMutation(
    `${process.env.API_URL}/MtCountry/Create`,
    creator /* options */,
  )
  const {
    data: dataFindById,
    ...
    props.itemSelected > 0
      ? `${process.env.API_URL}/MtCountry/FindById/${props.itemSelected}`
      : null,
    fetcher,
  )
  const updateMutation = useSWRMutation(
    dataFindById !== undefined && dataFindById.status == 200
      ? `${process.env.API_URL}/MtCountry/Update/${dataFindById.result.id}`
      : null,
    updater /* options */,
  )
  ...
  const savePais = async (values: any): Promise<any> => {
    let toastId
    try {
