...
export default function Administracion() {
  ...
  const { data, mutate, isLoading } = useSWR(
    `${process.env.API_URL}/MtAdministration/GetAllPaginated?Page=${pageIndex}&PageSize=${pageSize}&SearchByProp=Code&SearchCriteria=${searchInput}&MtCountryId=${filtroPais}`,
    fetcher,
  )
  const mutation = useSWRMutation(
    `${process.env.API_URL}/MtAdministration/DeleteAll`,
    deleter,
  )
  ...
  const breadcrumbs = [
    ...
    { label: 'Administracion', link: null },
  ]
    ...
    <main>
        ...
        <div className="mx-auto max-w-screen-xl px-4 pb-8 pt-8">
          <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
            <Table
              ...
              descripcion="Repositorio de administraciones para las unidades de negocio."
              ...
              searchInputPlaceholder="Buscar administracion"
              filterText="Filtros"
              columsValues={
              ...
              columsLabels={['Administracion', 'pais']}
              columsActions={[
                {
                  label: 'Detalles',
      ...
      {modalNewItem ? (
        <ModalNewItem
          ...
          title={
            itemId === 0 ? 'Nueva administracion' : 'Actualizar administracion'
          }
          itemSelected={itemId}
          onMutate={async () => {
            await mutate()
      ...
      {modalDetail ? (
        <ModalDetail
          ...
          title="Detalles de administracion"
          itemSelected={itemId}
      ...
      {modalDelete ? (
        <ModalDeleteRow
          titulo={`Eliminar ${selectedRows.length > 1 ? 'administraciones' : 'administracion'}`}
          mensaje={`¿Estás seguro de que deseas eliminar ${selectedRows.length > 1 ? 'estas administraciones' : 'esta administracion'} de la lista? Una vez ${selectedRows.length > 1 ? 'eliminadas' : 'eliminada'}, no podrás recuperar los datos asociados. `}
