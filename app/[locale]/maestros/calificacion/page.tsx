...
export default function MaestroCalificacionActuacion() {
  ...
  const { data, mutate, isLoading } = useSWR(
    `${process.env.API_URL}/MtCalification/GetAllPaginated?Page=${pageIndex}&PageSize=${pageSize}&SearchByProp=Code&SearchCriteria=${searchInput}`,
    fetcher,
  )
  const mutation = useSWRMutation(
    `${process.env.API_URL}/MtCalification/DeleteAll`,
    deleter,
  )
    ...
    <main>
        ...
        <div className="mx-auto max-w-screen-xl px-4 pb-8 pt-8">
          <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
            <Table
              ...
              descripcion="Repositorio con las diferentes calificaciones."
              ...
              searchInputPlaceholder="Buscar calificacion"
      ...
      {modalNewItem ? (
        <ModalNewItem
          ...
          title={
            itemId === 0 ? 'Nueva calificacion' : 'Actualizar calificacion'
          }
          itemSelected={itemId}
          onMutate={async () => {
            await mutate()
      ...
      {modalDetail ? (
        <ModalDetail
          ...
          title="Detalles de calificacion"
          itemSelected={itemId}
      ...
      {modalDelete ? (
        <ModalDeleteRow
          titulo={`Eliminar ${selectedRows.length > 1 ? 'calificaciones' : 'calificacion'}`}
          mensaje={`¿Estás seguro de que deseas eliminar ${selectedRows.length > 1 ? 'estas calificaciones' : 'esta calificacion'} de la lista? Una vez ${selectedRows.length > 1 ? 'eliminadas' : 'eliminada'}, no podrás recuperar los datos asociados. `}
