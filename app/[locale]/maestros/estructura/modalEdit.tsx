...
export default function ModalNewItem(props: PropsModalNewItem) {
  const { trigger, error } = useSWRMutation(
    `${process.env.API_URL}/MtStructureType/Create`,
    creator /* options */,
  )
  const {
    data: dataFindById,
    ...
    props.itemSelected > 0
      ? `${process.env.API_URL}/MtStructureType/FindById/${props.itemSelected}`
      : null,
    fetcher,
  )
  const updateMutation = useSWRMutation(
    dataFindById !== undefined && dataFindById.status == 200
      ? `${process.env.API_URL}/MtStructureType/Update/${dataFindById.result.id}`
      : null,
    updater /* options */,
  )
  ...
  const saveEstructura = async (values: any): Promise<any> => {
    let toastId
    try {
