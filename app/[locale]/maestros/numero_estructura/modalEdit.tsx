...
export default function ModalNewItem(props: PropsModalNewItem) {
  const { trigger, error } = useSWRMutation(
    `${process.env.API_URL}/MtStructureNumber/Create`,
    creator /* options */,
  )
  const {
    data: dataFindById,
  ...
  const updateMutation = useSWRMutation(
    dataFindById !== undefined && dataFindById.status == 200
      ? `${process.env.API_URL}/MtStructureNumber/Update/${dataFindById.result.id}`
      : null,
    updater /* options */,
  )
  ...
  const saveNumeroEstructura = async (values: any): Promise<any> => {
    let toastId
    try {
