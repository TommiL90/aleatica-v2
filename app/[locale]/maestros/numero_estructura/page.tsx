...
export default function MaestroNumeroEstructuras() {
  ...
  const { data, mutate, isLoading } = useSWR(
    `${process.env.API_URL}/MtStructureNumber/GetAllPaginated?Page=${pageIndex}&PageSize=${pageSize}&SearchByProp=Code&SearchCriteria=${searchInput}`,
    fetcher,
  )
  const mutation = useSWRMutation(
    `${process.env.API_URL}/MtStructureNumber/DeleteAll`,
    deleter,
  )
  ...
  const breadcrumbs = [
    { label: 'Inicio', link: '/' },
    { label: 'Categoria de actuacion', link: null },
  ]
    ...
    <main>
      ...
      <section className="max-h-screen w-full bg-white dark:bg-gray-900 lg:m-auto lg:w-10/12">
        ...
        <div className="mx-auto max-w-screen-xl px-4 pb-8 pt-8">
          <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
            <Table
              titulo="Numero de estructuras"
              descripcion="Repositorio con numero de estructuras."
              ...
              searchInputPlaceholder="Buscar numero de estructura"
      ...
      {modalNewItem ? (
        <ModalNewItem
          ...
          title={
            itemId === 0
              ? 'Nuevo numero de estructura'
              : 'Actualizar numero de estructura'
          }
          itemSelected={itemId}
          onMutate={async () => {
            await mutate()
      ...
      {modalDetail ? (
        <ModalDetail
          ...
          title="Detalles de numero de estructura"
          itemSelected={itemId}
      ...
      {modalDelete ? (
        <ModalDeleteRow
          titulo={`Eliminar ${selectedRows.length > 1 ? 'elementos' : 'elemento'}`}
          mensaje={`¿Estás seguro de que deseas eliminar ${selectedRows.length > 1 ? 'estos elementos' : 'este elemento'} de la lista? Una vez ${selectedRows.length > 1 ? 'eliminadas' : 'eliminada'}, no podrás recuperar los datos asociados. `}
