...
const MaestroUnidadObra = () => {
  ...
  const { data, mutate, isLoading } = useSWR(
    `${process.env.API_URL}/MtUnitOfMeasurement/GetAllPaginated?Page=${pageIndex}&PageSize=${pageSize}&SearchByProp=Code&SearchCriteria=${searchInput}`,
    fetcher,
  )
  const mutation = useSWRMutation(
    `${process.env.API_URL}/MtUnitOfMeasurement/DeleteAll`,
    deleter,
  )
  ...
  const breadcrumbs = [
    { label: 'Inicio', link: '/' },
    { label: 'Unidades de obra', link: null },
  ]
  return (
    <main>
      {/* <!-- Start block --> */}
      <section className="max-h-screen w-full bg-white dark:bg-gray-900 lg:m-auto lg:w-10/12">
        ...
        <div className="mx-auto max-w-screen-xl px-4 pb-8 pt-8">
          <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
            <Table
              titulo="Unidades de obras"
              descripcion="Repositorio con las diferentes unidades de medidas de obra"
              ...
              searchInputPlaceholder="Buscar unidades"
      ...
      {modalNewItem ? (
        <ModalNewItem
          ...
          title={
            itemId === 0 ? 'Nueva unidad de obra' : 'Actualizar unidad de obra'
          }
          itemSelected={itemId}
          onMutate={async () => {
            await mutate()
      ...
      {modalDetail ? (
        <ModalDetail
          ...
          title="Detalles de unidad de obra"
          itemSelected={itemId}
      ...
      {modalDelete ? (
        <ModalDeleteRow
          titulo={`Eliminar ${selectedRows.length > 1 ? 'unidades' : 'unidad'}`}
...
export default MaestroUnidadObra
