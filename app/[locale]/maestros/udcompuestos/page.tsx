...
const getUnidadesSimples = (): UnidadCompuesta[] => []
      ...
      switch (col.columnId) {
        ...
        case 'modal':
          elem = {
            ...
            style: { color: '#666179' },
            enabled: true,
            size: item.unidadesSimples.length,
            id: item.id,
            onClick: () => {},
          }
          break
...
const getEmpty = (id: number = 1): UnidadCompuesta => ({
  idauto: id,
  id: 0,
  idUnidad: '',
...
const moreRows = (unidades: UnidadCompuesta[], exceed: number = 100) => [
  ...unidades,
...
export default function CatalogoUnidadesCompuestas() {
  ...
  const [item, setItem] = useState<UnidadCompuesta>(getEmpty)
  ...
  useEffect(() => {
    ...
    try {
      if (data != undefined && data.status == 200) {
        setUnidadesCompuestas(
          moreRows(
            data.result.map((item: any, idx: number) => ({
              idauto: idx + 1,
              id: item.id,
              idUnidad: item.compositeUdId,
              codigoCompuesta: item.code,
              nombreUnidadCompuesta: item.compositeUdName,
              descripcionUnidadCompuesta: item.description,
              counter: String(item.count),
        ...
        setUnidadesCompuestas(moreRows(getUnidadesSimples(), 1000))
      }
    } finally {
      setLoading(true)
      setTimeout(() => {
  ...
  return (
    <main>
      <section className="max-h-screen w-full bg-white dark:bg-gray-900 lg:m-auto lg:w-10/12">
        ...
        {/* <SpreadSheet
                    titulo="Catalogo de unidades compuestas"
                    descripcion="Repositorio de unidades compuestas."
                    searchInputPlaceholder="Buscar de unidades compuestas"
                    ...
                    onChangeRows={(items: UnidadCompuesta[])=> setUnidadesCompuestas(items)}
                    onChangeColumns={(columns: Column[])=> setColumns(columns)}
                    onChange={(changes: CellChange<any>[]) => setUnidadesCompuestas(applyChanges(
                      changes, 
                      unidades,
                      espRes !== undefined && espRes.status === 200 ?
                    ...
                    onImport={(dataImported: any, columnasMapeadas: any)=> {
                        ...
                        setUnidadesCompuestas(imported)
        ...
        {modal ? (
          <Modal
            ...
            options={
              udSimplesRes !== undefined && udSimplesRes.status === 200
                ? udSimplesRes.result.map((item: any) => ({
            ...
            unidadesSimples={item.unidadesSimples}
            onChange={(values: any[]) =>
              setUnidadesCompuestas(
        ...
        {modalDelete ? (
          <ModalDeleteRow
            ...
            mensaje={`¿Estás seguro de que deseas eliminar esta unidad compuesta de la lista? Una vez eliminada, no podrás recuperar los datos asociados. `}
            onClose={() => setModalDelete(false)}
            onDelete={async () => {
              if (deleteItem > 0) {
                const res = await fetch(
                  `${process.env.API_URL}/CompositeCatalog/Delete/${deleteItem}`,
                  {
                    method: 'DELETE',
                    headers: {
                      'Content-Type': 'application/json',
          ...
          <div className="relative bg-white  shadow dark:bg-gray-700">
            ...
            <div className="space-y-6 p-6">
              <Simples
                ...
                unidadesSimples={props.unidadesSimples}
...
function Simples(props: SimplesProps) {
  ...
  for (let i = 0; i < props.unidadesSimples.length; i++) {
  ...
  const handleAgregarUdSimple = (e: any, id: any) => {
  ...
  const handleEliminarUdSimple = (e: any, id: number) => {
    if (id == 0) {
      return
    }
  ...
  const EmptyInputs = (
    ...
    >
      ...
      <td className="w-1/12 px-6 py-4 text-right">
        <button
          type="button"
          onClick={(e) => handleAgregarUdSimple(e, 0)}
      ...
      <td className="w-1/12 px-6 py-4 text-right">
        <button
          type="button"
          onClick={(e) => handleEliminarUdSimple(e, 0)}
                  ...
                  .map((item, index) => (
                    ...
                    >
                      ...
                      <td className="w-1/12 px-6 py-4 text-right">
                        <button
                          type="button"
                          onClick={(e) => handleAgregarUdSimple(e, item['id'])}
                          className="font-medium text-blue-600 hover:underline dark:text-blue-500"
                        >
                          <FaPlus />
                        </button>
                      ...
                      <td className="w-1/12 px-6 py-4 text-right">
                        <button
                          type="button"
                          onClick={(e) => handleEliminarUdSimple(e, item['id'])}
                          className="font-medium text-red-600 hover:underline dark:text-red-500"
                        >
                          <FaRegTrashAlt />
                        </button>
