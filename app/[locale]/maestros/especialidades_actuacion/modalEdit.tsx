...
import IndicadorEspecialidadForm from '@/components/forms/IndicadorEspecialidadForm'
...
const creator = async (
  ...
  {
    arg,
  }: { arg: { code: string; name: string; mtActionSubCategoryId: string } },
) => {
  return fetch(url, {
    method: 'POST',
    headers: {
...
export default function ModalNewItem(props: PropsModalNewItem) {
  ...
  const { trigger, error } = useSWRMutation(
    `${process.env.API_URL}/MtSpecialtyAction/Create`,
    creator /* options */,
  )
  const {
    data: dataFindById,
    ...
    props.itemSelected > 0
      ? `${process.env.API_URL}/MtSpecialtyAction/FindById/${props.itemSelected}`
      : null,
    fetcher,
  )
  const updateMutation = useSWRMutation(
    dataFindById !== undefined && dataFindById.status == 200
      ? `${process.env.API_URL}/MtSpecialtyAction/Update/${dataFindById.result.id}`
      : null,
    updater /* options */,
  )

  const saveEspecialidadActuacion = async (values: any): Promise<any> => {
    let toastId
    try {
      ...
      const item: any = {
        ...
        name: values.nombre,
        code: values.codigo,
        mtSubCategoryActionId: values.subcategoria,
      }
                      ...
                      initValue={
                        dataFindById !== undefined && dataFindById.status == 200
                          ? {
                              ...
                              subcategoria:
                                dataFindById.result.mtSubCategoryActionId,
                              subCategoriaNombre:
                                dataFindById.result.mtSubCategoryAction,
                            }
                          : null
                      }
                      buttonText="Guardar"
                      onSubmit={saveEspecialidadActuacion}
                    />
                  )}
                </div>
              </div>
