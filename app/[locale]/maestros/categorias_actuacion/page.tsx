...
export default function MaestroCategoriaActuacion() {
  ...
  const { data, mutate, isLoading } = useSWR(
    `${process.env.API_URL}/MtActionCategory/GetAllPaginated?Page=${pageIndex}&PageSize=${pageSize}&SearchByProp=Code&SearchCriteria=${searchInput}`,
    fetcher,
  )
  const mutation = useSWRMutation(
    `${process.env.API_URL}/MtActionCategory/DeleteAll`,
    deleter,
  )
    ...
    <main>
      ...
      <section className="max-h-screen w-full bg-white dark:bg-gray-900 lg:m-auto lg:w-10/12">
        ...
        <div className="mx-auto max-w-screen-xl px-4 pb-8 pt-8">
          <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
            <Table
              titulo="Categoria de actuacion"
              descripcion="Repositorio con las diferentes categorias de actuacion."
              ...
              searchInputPlaceholder="Buscar categoria actuacion"
      ...
      {modalNewItem ? (
        <ModalNewItem
          ...
          title={itemId === 0 ? 'Nueva categoria' : 'Actualizar categoria'}
          itemSelected={itemId}
          onMutate={async () => {
            await mutate()
            if (itemId !== 0) {
