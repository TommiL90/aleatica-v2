...
export default function ModalNewItem(props: PropsModalNewItem) {
  const { trigger, error } = useSWRMutation(
    `${process.env.API_URL}/MtActionCategory/Create`,
    creator /* options */,
  )
  const {
    data: dataFindById,
    ...
    props.itemSelected > 0
      ? `${process.env.API_URL}/MtActionCategory/FindById/${props.itemSelected}`
      : null,
    fetcher,
  )
  const updateMutation = useSWRMutation(
    dataFindById !== undefined && dataFindById.status == 200
      ? `${process.env.API_URL}/MtActionCategory/Update/${dataFindById.result.id}`
      : null,
    updater /* options */,
  )

  const saveCategoriaActuacion = async (values: any): Promise<any> => {
    let toastId
    try {
                ...
                <div className="space-y-6 p-6">
                  {isValidating ? (
                    ...
                    <IndicadorForm
                      ...
                      buttonText="Guardar"
                      onSubmit={saveCategoriaActuacion}
                    />
                  )}
                </div>
              </div>
