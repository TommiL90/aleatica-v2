...
export default function ModalNewItem(props: PropsModalNewItem) {
  const { trigger, error } = useSWRMutation(
    `${process.env.API_URL}/MtSubCategoryAction/Create`,
    creator /* options */,
  )
  const {
    data: dataFindById,
    ...
    props.itemSelected > 0
      ? `${process.env.API_URL}/MtSubCategoryAction/FindById/${props.itemSelected}`
      : null,
    fetcher,
  )
  const updateMutation = useSWRMutation(
    dataFindById !== undefined && dataFindById.status == 200
      ? `${process.env.API_URL}/MtSubCategoryAction/Update/${dataFindById.result.id}`
      : null,
    updater /* options */,
  )
  ...
  const saveSeguimiento = async (values: any): Promise<any> => {
    let toastId
    try {
