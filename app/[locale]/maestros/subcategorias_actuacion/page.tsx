...
export default function MaestroSubCategoriaActuacion() {
  ...
  const { data, mutate, isLoading } = useSWR(
    `${process.env.API_URL}/MtSubCategoryAction/GetAllPaginated?Page=${pageIndex}&PageSize=${pageSize}&SearchByProp=Code&SearchCriteria=${searchInput}`,
    fetcher,
  )
  const mutation = useSWRMutation(
    `${process.env.API_URL}/MtSubCategoryAction/DeleteAll`,
    deleter,
  )
  ...
  const breadcrumbs = [
    { label: 'Inicio', link: '/' },
    { label: 'Subcategoria de actuacion', link: null },
  ]
    ...
    <main>
      ...
      <section className="max-h-screen w-full bg-white dark:bg-gray-900 lg:m-auto lg:w-10/12">
        ...
        <div className="mx-auto max-w-screen-xl px-4 pb-8 pt-8">
          <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
            <Table
              titulo="Subcategoria de actuacion"
              descripcion="Repositorio con las diferentes subcategorias de actuacion"
              ...
              searchInputPlaceholder="Buscar subcategoria actuacion"
              filterText="Subcategoria"
              columsValues={
      ...
      {modalNewItem ? (
        <ModalNewItem
          ...
          title={
            itemId === 0 ? 'Nueva subcategoria' : 'Actualizar subcategoria'
          }
          itemSelected={itemId}
          onMutate={async () => {
            await mutate()
      ...
      {modalDetail ? (
        <ModalDetail
          ...
          title="Detalles de subcategoria"
          itemSelected={itemId}
      ...
      {modalDelete ? (
        <ModalDeleteRow
          titulo={`Eliminar ${selectedRows.length > 1 ? 'subcategorias' : 'subcategoria'}`}
          mensaje={`¿Estás seguro de que deseas eliminar ${selectedRows.length > 1 ? 'estas subcategorias' : 'esta subcategoria'} de la lista? Una vez ${selectedRows.length > 1 ? 'eliminadas' : 'eliminada'}, no podrás recuperar los datos asociados. `}
