...
export default function ModalNewItem(props: PropsModalNewItem) {
  const { trigger, error } = useSWRMutation(
    `${process.env.API_URL}/MtProjectCategory/Create`,
    creator /* options */,
  )
  const {
    data: dataFindById,
    ...
    props.itemSelected > 0
      ? `${process.env.API_URL}/MtProjectCategory/FindById/${props.itemSelected}`
      : null,
    fetcher,
  )
  const updateMutation = useSWRMutation(
    dataFindById !== undefined && dataFindById.status == 200
      ? `${process.env.API_URL}/MtProjectCategory/Update/${dataFindById.result.id}`
      : null,
    updater /* options */,
  )
  ...
  const saveCategoriaProyecto = async (values: any): Promise<any> => {
    let toastId
    try {
                ...
                <div className="space-y-6 p-6">
                  {isValidating ? (
                    ...
                    <IndicadorForm
                      ...
                      buttonText="Guardar"
                      onSubmit={saveCategoriaProyecto}
                    />
                  )}
                </div>
              </div>
