...
export default function MaestroCategoriaProyecto() {
  ...
  const { data, mutate, isLoading } = useSWR(
    `${process.env.API_URL}/MtProjectCategory/GetAllPaginated?Page=${pageIndex}&PageSize=${pageSize}&SearchByProp=Code&SearchCriteria=${searchInput}`,
    fetcher,
  )
  const mutation = useSWRMutation(
    `${process.env.API_URL}/MtProjectCategory/DeleteAll`,
    deleter,
  )
  ...
  const breadcrumbs = [
    { label: 'Inicio', link: '/' },
    { label: 'Categorias de proyecto', link: null },
  ]
    ...
    <main>
      ...
      <section className="max-h-screen w-full bg-white dark:bg-gray-900 lg:m-auto lg:w-10/12">
        ...
        <div className="mx-auto max-w-screen-xl px-4 pb-8 pt-8">
          <div className="relative overflow-x-auto  pb-4 shadow-md sm:rounded-lg">
            <Table
              titulo="Categoria de proyectos"
              descripcion="Repositorio con las diferentes categorias de proyectos."
              ...
              searchInputPlaceholder="Buscar categoria proyectos"
      ...
      {modalNewItem ? (
        <ModalNewItem
          ...
          title={
            itemId === 0
              ? 'Nueva categoria de proyecto'
              : 'Actualizar categoria de proyecto'
          }
          itemSelected={itemId}
          onMutate={async () => {
            await mutate()
      ...
      {modalDetail ? (
        <ModalDetail
          ...
          title="Detalles de categoria de proyecto"
          itemSelected={itemId}
      ...
      {modalDelete ? (
        <ModalDeleteRow
          titulo={`Eliminar ${selectedRows.length > 1 ? 'categorias' : 'categoria'}`}
          mensaje={`¿Estás seguro de que deseas eliminar ${selectedRows.length > 1 ? 'estas categorias' : 'esta categoria'} de la lista? Una vez ${selectedRows.length > 1 ? 'eliminadas' : 'eliminada'}, no podrás recuperar los datos asociados. `}
