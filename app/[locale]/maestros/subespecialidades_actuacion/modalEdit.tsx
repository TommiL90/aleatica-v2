...
export default function ModalNewItem(props: PropsModalNewItem) {
  ...
  const {
    ...
    props.itemSelected > 0
      ? `${process.env.API_URL}/MtSubspeciality/FindById/${props.itemSelected}`
      : null,
    fetcher,
  )
  const updateMutation = useSWRMutation(
    dataFindById !== undefined && dataFindById.status == 200
      ? `${process.env.API_URL}/MtSubspeciality/Update/${dataFindById.result.id}`
      : null,
    updater /* options */,
  )
                ...
                <div className="space-y-6 p-6">
                  {isValidating ? (
                    ...
                    <IndicadorSubEspecialidadForm
                      ...
                      initValue={
                        props.itemSelected > 0 &&
                        dataFindById !== undefined &&
                        dataFindById.status == 200
                          ? {
                              ...
                              padre: dataFindById.result.fatherId,
                            }
                          : null
                      }
                      buttonText="Guardar subespecialidad"
