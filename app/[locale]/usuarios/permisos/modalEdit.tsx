...
import PermisoForm from '@/components/forms/usuarios/permisoForm'
...
const creator = async (
  url: string,
  { arg }: { arg: { module: string; description: string } },
) => {
  return fetch(url, {
    method: 'POST',
    headers: {
...
const updater = async (
  url: string,
  { arg }: { arg: { id: number; module: string; description: string } },
) => {
  return fetch(url, {
    method: 'PUT',
    headers: {
...
export default function ModalNewItem(props: PropsModalNewItem) {
  const { trigger, error } = useSWRMutation(
    `${process.env.API_URL}/Permission/Create`,
    creator /* options */,
  )
  const {
    data: dataFindById,
    ...
    props.itemSelected > 0
      ? `${process.env.API_URL}/Permission/FindById/${props.itemSelected}`
      : null,
    fetcher,
  )
  const updateMutation = useSWRMutation(
    dataFindById !== undefined && dataFindById.status == 200
      ? `${process.env.API_URL}/Permission/Update/${dataFindById.result.id}`
      : null,
    updater /* options */,
  )
  ...
  const savePermiso = async (values: any): Promise<any> => {
    console.log(values)
    let toastId
    try {
                      ...
                      initValue={
                        dataFindById !== undefined && dataFindById.status == 200
                          ? {
                              ...
                              modulo: dataFindById.result.module,
                              descripcion: dataFindById.result.description,
                            }
                          : null
                      }
                      buttonText="Guardar"
                      onSubmit={savePermiso}
                    />
                  )}
                </div>
              </div>
