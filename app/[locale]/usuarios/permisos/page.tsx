...
const mapFieldsOrdered = [
  ...
  { fieldName: 'module', fieldType: 'string' },
...
export default function RolesList() {
  ...
  const { data, mutate, isLoading, error } = useSWR(
    `${process.env.API_URL}/Permission/GetAllPaginated?Page=${pageIndex}&PageSize=${pageSize}&SearchByProp=Code&SearchCriteria=${searchInput}`,
    fetcher,
  )
  const mutation = useSWRMutation(
    `${process.env.API_URL}/Permission/DeleteAll`,
    deleter,
  )
    ...
    <main>
        ...
        <div className="mx-auto max-w-screen-xl px-4 pb-8 pt-8">
          <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
            <Table
              ...
              descripcion="Repositorio con los diferentes permisos de usuarios en el sistema"
              ...
              searchInputPlaceholder="Buscar permiso"
              filterText="Filtros"
              columsValues={
              ...
              columsLabels={['Modulo de permiso', 'Descripcion']}
              columsActions={[
                {
                  label: 'Detalles',
      ...
      {modalNewItem ? (
        <ModalNewItem
          ...
          title={itemId === 0 ? 'Nuevo permiso' : 'Actualizar permiso'}
          itemSelected={itemId}
          onMutate={async () => {
            await mutate()
            if (itemId !== 0) {
      ...
      {modalDelete ? (
        <ModalDeleteRow
          titulo={`Eliminar ${selectedRows.length > 1 ? 'permisos' : 'permiso'}`}
          mensaje={`¿Estás seguro de que deseas eliminar ${selectedRows.length > 1 ? 'estos permisos' : 'este permiso'} de la lista? Una vez ${selectedRows.length > 1 ? 'eliminados' : 'eliminado'}, no podrás recuperar los datos asociados. `}
