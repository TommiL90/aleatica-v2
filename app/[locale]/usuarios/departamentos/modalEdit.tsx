...
import NameandUnitForm from '@/components/forms/usuarios/formNameandUnit'
...
const creator = async (
  url: string,
  { arg }: { arg: { name: string; mtBusinessUnitId: string } },
) => {
  return fetch(url, {
    method: 'POST',
    headers: {
...
const updater = async (
  url: string,
  { arg }: { arg: { id: number; name: string; mtBusinessUnitId: string } },
) => {
  return fetch(url, {
    method: 'PUT',
    headers: {
...
export default function ModalNewItem(props: PropsModalNewItem) {
  ...
  const { trigger, error } = useSWRMutation(
    `${process.env.API_URL}/MtDepartment/Create`,
    creator /* options */,
  )
  const {
    data: dataFindById,
    ...
    props.itemSelected > 0
      ? `${process.env.API_URL}/MtDepartment/FindById/${props.itemSelected}`
      : null,
    fetcher,
  )
  const updateMutation = useSWRMutation(
    dataFindById !== undefined && dataFindById.status == 200
      ? `${process.env.API_URL}/MtDepartment/Update/${dataFindById.result.id}`
      : null,
    updater /* options */,
  )

  console.log(dataFindById)
  const saveDepartamento = async (values: any): Promise<any> => {
    let toastId
    try {
