...
export default function RolesList() {
  ...
  const { data, mutate, isLoading, error } = useSWR(
    `${process.env.API_URL}/Rol/GetAllPaginated?Page=${pageIndex}&PageSize=${pageSize}&SearchByProp=Code&SearchCriteria=${searchInput}`,
    fetcher,
  )
  const mutation = useSWRMutation(
    `${process.env.API_URL}/Rol/DeleteAll`,
    deleter,
  )
    ...
    <main>
        ...
        <div className="mx-auto max-w-screen-xl px-4 pb-8 pt-8">
          <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
            <Table
              ...
              descripcion="Repositorio con los diferentes roles de usuarios en el sistema"
              ...
              searchInputPlaceholder="Buscar rol"
              filterText="Filtros"
              columsValues={
      ...
      {modalNewItem ? (
        <ModalNewItem
          ...
          title={itemId === 0 ? 'Nuevo rol' : 'Actualizar rol'}
          itemSelected={itemId}
          onMutate={async () => {
            await mutate()
            if (itemId !== 0) {
      ...
      {modalDelete ? (
        <ModalDeleteRow
          titulo={`Eliminar ${selectedRows.length > 1 ? 'roles' : 'rol'}`}
          mensaje={`¿Estás seguro de que deseas eliminar ${selectedRows.length > 1 ? 'estos roles' : 'este rol'} de la lista? Una vez ${selectedRows.length > 1 ? 'eliminados' : 'eliminado'}, no podrás recuperar los datos asociados. `}
