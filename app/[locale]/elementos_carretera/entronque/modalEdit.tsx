...
const creator = async (
  url: string,
  { arg }: { arg: { code: string; name: string } },
) => {
  return fetch(url, {
    method: 'POST',
    headers: {
...
export default function ModalNewItem(props: PropsModalNewItem) {
  const { trigger, error } = useSWRMutation(
    `${process.env.API_URL}/MtHighwayIntersection/Create`,
    creator /* options */,
  )
  const {
    data: dataFindById,
    ...
    props.itemSelected > 0
      ? `${process.env.API_URL}/MtHighwayIntersection/FindById/${props.itemSelected}`
      : null,
    fetcher,
  )
  const updateMutation = useSWRMutation(
    dataFindById !== undefined && dataFindById.status == 200
      ? `${process.env.API_URL}/MtHighwayIntersection/Update/${dataFindById.result.id}`
      : null,
    updater /* options */,
  )
  ...
  const saveEntronque = async (values: any): Promise<any> => {
    let toastId
    try {
      ...
      const item: any = {
        id:
          dataFindById !== undefined && dataFindById.status == 200
            ...
            : 0,
        name: values.nombreEntronque,
        code: values.codigoEntronque,
      }
              ...
              <div className="relative bg-white  shadow dark:bg-gray-700">
                ...
                <div className="space-y-6 p-6">
                  {isValidating ? (
                    ...
                    <CatalogoEntronquesForm
                      initValue={
                        dataFindById !== undefined && dataFindById.status == 200
                          ? {
                              id: dataFindById.result.id,
                              nombreEntronque: dataFindById.result.name,
                              codigoEntronque: dataFindById.result.code,
                            }
                          : null
                      }
                      buttonText="Guardar"
                      onSubmit={saveEntronque}
                    />
                  )}
                </div>
              </div>
