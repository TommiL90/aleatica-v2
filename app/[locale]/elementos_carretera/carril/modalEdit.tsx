...
import CatalogoCalzadaForm from '@/components/forms/catalogoCalzada'
import CatalogoCarrilForm from '@/components/forms/catalogoCarril'
...
export default function ModalNewItem(props: PropsModalNewItem) {
  const { trigger, error } = useSWRMutation(
    `${process.env.API_URL}/MtHighwayLane/Create`,
    creator /* options */,
  )
  const {
    data: dataFindById,
    ...
    props.itemSelected > 0
      ? `${process.env.API_URL}/MtHighwayLane/FindById/${props.itemSelected}`
      : null,
    fetcher,
  )
  const updateMutation = useSWRMutation(
    dataFindById !== undefined && dataFindById.status == 200
      ? `${process.env.API_URL}/MtHighwayLane/Update/${dataFindById.result.id}`
      : null,
    updater /* options */,
  )
  ...
  const saveCarril = async (values: any): Promise<any> => {
    let toastId
    try {
      ...
      const item: any = {
        id:
          dataFindById !== undefined && dataFindById.status == 200
            ...
            : 0,
        name: values.nombreCarril,
        code: values.codigoCarril,
      }
                      ...
                      initValue={
                        dataFindById !== undefined && dataFindById.status == 200
                          ? {
                              ...
                              nombreCarril: dataFindById.result.name,
                              // codigoCarril: data.result.code,
                            }
                          : null
                      }
                      buttonText="Guardar"
