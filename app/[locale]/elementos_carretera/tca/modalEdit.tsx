...
export default function ModalNewItem(props: PropsModalNewItem) {
  ...
  const { trigger, error } = useSWRMutation(
    `${process.env.API_URL}/MtAccidentRoadSection/Create`,
    creator /* options */,
  )
  const {
    data: dataFindById,
    ...
    props.itemSelected > 0
      ? `${process.env.API_URL}/MtAccidentRoadSection/FindById/${props.itemSelected}`
      : null,
    fetcher,
  )
  const updateMutation = useSWRMutation(
    dataFindById !== undefined && dataFindById.status == 200
      ? `${process.env.API_URL}/MtAccidentRoadSection/Update/${dataFindById.result.id}`
      : null,
    updater /* options */,
  )
                      ...
                      tramos={
                        tramoRes !== undefined && tramoRes.status === 200
                          ? tramoRes.result.map((item: any) => ({
                              ...
                              entronques:
                                item.mtRoadSectionMtHighwayIntersections.map(
                                  (entronque: any) => ({
                      ...
                      initValue={
                        dataFindById !== undefined && dataFindById.status == 200
                          ? {
                              ...
                              tramo: dataFindById.result.mtRoadSectionId,
                              entronque:
                                dataFindById.result.mtHighwayIntersectionId,
                              gaza: dataFindById.result.mtSlipLaneRoadId,
                              descripcionTCA: dataFindById.result.description,
                              cadenamientoInicial:
                                dataFindById.result.initialNumber,
                              cadenamientoFinal: dataFindById.result.endNumber,
                            }
                          : null
                      }
                      buttonText="Guardar"
