...
const mapFieldsOrdered = [{ fieldName: 'name', fieldType: 'string' }]
...
export default function CatalogoActuaciones() {
  ...
  const { data, mutate, isLoading } = useSWR(
    `${process.env.API_URL}/MtAccidentRoadSection/GetAllPaginated?Page=${pageIndex}&PageSize=${pageSize}&SearchByProp=Code&SearchCriteria=${searchInput}`,
    fetcher,
  )
  const mutation = useSWRMutation(
    `${process.env.API_URL}/MtAccidentRoadSection/DeleteAll`,
    deleter,
  )
  ...
  const breadcrumbs = [
    { label: 'Inicio', link: '/' },
    { label: 'Tramos de concentracion de accidentes', link: null },
  ]
    ...
    <main>
      ...
      <section className="max-h-screen w-full bg-white dark:bg-gray-900 lg:m-auto lg:w-10/12">
        ...
        <div className="mx-auto max-w-screen-xl px-4 pb-8 pt-8">
          <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
            <Table
              titulo="Tramos de concentracion de accidentes"
              descripcion="Repositorio de tramos de concentracion de accidentes."
              ...
              searchInputPlaceholder="Buscar tramo de accidentes"
      ...
      {modalNewItem ? (
        <ModalNewItem
          ...
          title={itemId === 0 ? 'Nuevo TCA' : 'Actualizar TCA'}
          itemSelected={itemId}
          onMutate={async () => {
            await mutate()
            if (itemId !== 0) {
