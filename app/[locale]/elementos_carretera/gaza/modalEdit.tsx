...
import CatalogoGazaForm from '@/components/forms/catalogoGaza'
...
export default function ModalNewItem(props: PropsModalNewItem) {
  const { trigger, error } = useSWRMutation(
    `${process.env.API_URL}/MtSlipLaneRoad/Create`,
    creator /* options */,
  )
  const {
    data: dataFindById,
    ...
    props.itemSelected > 0
      ? `${process.env.API_URL}/MtSlipLaneRoad/FindById/${props.itemSelected}`
      : null,
    fetcher,
  )
  const updateMutation = useSWRMutation(
    dataFindById !== undefined && dataFindById.status == 200
      ? `${process.env.API_URL}/MtSlipLaneRoad/Update/${dataFindById.result.id}`
      : null,
    updater /* options */,
  )
  ...
  const saveGaza = async (values: any): Promise<any> => {
    let toastId
    try {
      ...
      const item: any = {
        id:
          dataFindById !== undefined && dataFindById.status == 200
            ...
            : 0,
        name: values.nombreGaza,
        code: values.codigoGaza,
      }
                      ...
                      initValue={
                        dataFindById !== undefined && dataFindById.status == 200
                          ? {
                              ...
                              nombreGaza: dataFindById.result.name,
                              codigoGaza: dataFindById.result.code,
                            }
                          : null
                      }
                      buttonText="Guardar"
