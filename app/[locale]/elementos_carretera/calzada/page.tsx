...
export default function Calzadas() {
  ...
  const { data, mutate, isLoading } = useSWR(
    `${process.env.API_URL}/MtHighwayLane/GetAllPaginated?Page=${pageIndex}&PageSize=${pageSize}&SearchByProp=Name&SearchCriteria=${searchInput}`,
    fetcher,
  )
  const mutation = useSWRMutation(
    `${process.env.API_URL}/MtHighwayLane/DeleteAll`,
    deleter,
  )
    ...
    <main>
      ...
      <section className="max-h-screen w-full bg-white dark:bg-gray-900 lg:m-auto lg:w-10/12">
        ...
        <div className="mx-auto max-w-screen-xl px-4 pb-8 pt-8">
          <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
            <Table
              titulo="Catalogo de calzadas"
              descripcion="Repositorio de calzadas."
              ...
              searchInputPlaceholder="Catalogo de calzadas"
              filterText="Unidades de negocios"
              columsValues={
      ...
      {modalNewItem ? (
        <ModalNewItem
          ...
          title={itemId === 0 ? 'Nueva calzada' : 'Actualizar calzada'}
          itemSelected={itemId}
          onMutate={async () => {
            await mutate()
            if (itemId !== 0) {
      ...
      {modalDelete ? (
        <ModalDeleteRow
          titulo={`Eliminar ${selectedRows.length > 1 ? 'calzadas' : 'calzada'}`}
          mensaje={`¿Estás seguro de que deseas eliminar ${selectedRows.length > 1 ? 'estas calzadas' : 'esta calzada'} de la lista? Una vez ${selectedRows.length > 1 ? 'eliminadas' : 'eliminada'}, no podrás recuperar los datos asociados. `}
