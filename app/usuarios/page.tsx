...
const mapFieldsOrdered = [
  ...
  { fieldName: 'profileName', fieldType: 'string' },
...
export default function UsuariosList() {
  ...
  const { data, mutate, isLoading, error } = useSWR(
    `${process.env.API_URL}/User/GetAllPaginated?Page=${pageIndex}&PageSize=${pageSize}&SearchByProp=Code&SearchCriteria=${searchInput}`,
    fetcher,
  )
  const mutation = useSWRMutation(
    `${process.env.API_URL}/User/DeleteAll`,
    deleter,
  )
  const router = useRouter()

  const [modalNewItem, setModalNewItem] = useStateCallback(false)
  const [modalChangePassword, setModalChangePassword] = useStateCallback(false)
    ...
    <main>
      ...
      <section className="max-h-screen w-full bg-white dark:bg-gray-900 lg:m-auto lg:w-10/12">
        ...
        <div className="mx-auto max-w-screen-xl px-4 pb-8 pt-8">
          <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
            <Table
              titulo="Usuarios"
              descripcion="Repositorio con los diferentes usuarios en el sistema"
              ...
              columsLabels={['Nombre', 'Email', 'username', 'Roles']}
              columsActions={[
                {
                  label: 'Detalles',
      ...
      {modalNewItem ? (
        <ModalNewItem
          ...
          title={itemId === 0 ? 'Nuevo usuario' : 'Actualizar usuario'}
          itemSelected={itemId}
          onMutate={async () => {
            await mutate()
            if (itemId !== 0) {
      ...
      {modalDelete ? (
        <ModalDeleteRow
          titulo={`Eliminar ${selectedRows.length > 1 ? 'usuarios' : 'usuario'}`}
          mensaje={`¿Estás seguro de que deseas eliminar ${selectedRows.length > 1 ? 'estos usuarios' : 'este usuario'} de la lista? Una vez ${selectedRows.length > 1 ? 'eliminados' : 'eliminado'}, no podrás recuperar los datos asociados. `}
