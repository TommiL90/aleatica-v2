...
import IndicadorDesempenoForm from '@/components/forms/indicadorDesempeno'
...
export default function ModalNewItem(props: PropsModalNewItem) {
  ...
  const { data, mutate } = useSWR(
    params
      ? `${process.env.API_URL}/MRPerformanceIndicator/FindById/${params.id}`
      : null,
    fetcher,
  )
  ...
  const { data: especialityRes, isLoading: loadinEspecialidades } = useSWR(
  ...
  const {
    ...
    props.itemSelected > 0
      ? `${process.env.API_URL}/MRPerformanceIndicator/FindById/${props.itemSelected}`
      : null,
    fetcher,
  )
  const { trigger, error } = useSWRMutation(
    `${process.env.API_URL}/MRPerformanceIndicator/Create`,
    creator /* options */,
  )
  const updateMutation = useSWRMutation(
    dataFindById !== undefined && dataFindById.status == 200
      ? `${process.env.API_URL}/MRPerformanceIndicator/Update/${dataFindById.result.id}`
      : null,
    updater /* options */,
  )
  ...
  const saveDesempenno = async (values: any): Promise<any> => {
    let toastId
    try {
      ...
      const item: any = {
        ...
        mtSpecialtyActionId: Number(values.especialidad),
      }
            ...
            <div className="max-w-7xlxl relative mx-auto max-h-full w-[700px]">
              <div className="relative bg-white  shadow dark:bg-gray-700">
                ...
                <div className="space-y-6 p-6">
                  {isValidating || loadinEspecialidades ? (
                    <Loading label={''} />
                  ) : (
                    <IndicadorDesempenoForm
                      unidadNegocio={
                        businessUnitRes !== undefined &&
                        businessUnitRes.status === 200
                      ...
                      subCategoria={
                        subcategoryRes !== undefined &&
                        subcategoryRes.status === 200
                      ...
                      especialidad={
                        especialityRes !== undefined &&
                        especialityRes.status === 200
                      ...
                      initValue={
                        dataFindById !== undefined && dataFindById.status == 200
                          ? {
                              ...
                              subCategoria:
                                dataFindById.result.mtActionSubCategoryId,
                              especialidad:
                                dataFindById.result.mtSpecialtyActionId,
                            }
                          : null
                      }
                      buttonText="Guardar"
                      onSubmit={saveDesempenno}
                    />
                  )}
                </div>
              </div>
