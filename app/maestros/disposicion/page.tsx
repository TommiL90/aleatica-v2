...
export default function MaestroCategoriaDisposicion() {
  ...
  const { data, mutate, isLoading } = useSWR(
    `${process.env.API_URL}/MtDisposition/GetAllPaginated?Page=${pageIndex}&PageSize=${pageSize}&SearchByProp=Code&SearchCriteria=${searchInput}`,
    fetcher,
  )
  const mutation = useSWRMutation(
    `${process.env.API_URL}/MtDisposition/DeleteAll`,
    deleter,
  )
    ...
    <main>
        ...
        <div className="mx-auto max-w-screen-xl px-4 pb-8 pt-8">
          <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
            <Table
              ...
              descripcion="Repositorio con las disposiciones"
              ...
              searchInputPlaceholder="Buscar disposicion"
      ...
      {modalNewItem ? (
        <ModalNewItem
          ...
          title={itemId === 0 ? 'Nueva disposicion' : 'Actualizar disposicion'}
          itemSelected={itemId}
          onMutate={async () => {
            await mutate()
            if (itemId !== 0) {
      ...
      {modalDelete ? (
        <ModalDeleteRow
          titulo={`Eliminar ${selectedRows.length > 1 ? 'disposiciones' : 'disposicion'}`}
          mensaje={`¿Estás seguro de que deseas eliminar ${selectedRows.length > 1 ? 'estas disposiciones' : 'esta disposicion'} de la lista? Una vez ${selectedRows.length > 1 ? 'eliminadas' : 'eliminada'}, no podrás recuperar los datos asociados. `}
