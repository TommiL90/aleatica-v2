...
export default function MaestroAmbito() {
  ...
  const { data, mutate, isLoading } = useSWR(
    `${process.env.API_URL}/MtScopeOfAction/GetAllPaginated?Page=${pageIndex}&PageSize=${pageSize}&SearchByProp=Code&SearchCriteria=${searchInput}`,
    fetcher,
  )
  const mutation = useSWRMutation(
    `${process.env.API_URL}/MtScopeOfAction/DeleteAll`,
    deleter,
  )
  ...
  const breadcrumbs = [
    { label: 'Inicio', link: '/' },
    { label: 'Ambito de actuacion', link: null },
  ]
    ...
    <main>
      ...
      <section className="max-h-screen w-full bg-white dark:bg-gray-900 lg:m-auto lg:w-10/12">
        ...
        <div className="mx-auto max-w-screen-xl px-4 pb-8 pt-8">
          <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
            <Table
              titulo="Ambito de actuacion"
              descripcion="Repositorio de ambitos de actuacion"
              ...
              searchInputPlaceholder="Buscar ambito actuacion"
      ...
      {modalNewItem ? (
        <ModalNewItem
          ...
          title={itemId === 0 ? 'Nuevo ambito' : 'Actualizar ambito'}
          itemSelected={itemId}
          onMutate={async () => {
            await mutate()
            if (itemId !== 0) {
      ...
      {modalDetail ? (
        <ModalDetail
          ...
          title="Detalles de ambito de actuacion"
          itemSelected={itemId}
      ...
      {modalDelete ? (
        <ModalDeleteRow
          titulo={`Eliminar ${selectedRows.length > 1 ? 'ambitos' : 'ambito'}`}
          mensaje={`¿Estás seguro de que deseas eliminar ${selectedRows.length > 1 ? 'estos ambitos' : 'este ambito'} de la lista? Una vez ${selectedRows.length > 1 ? 'eliminadas' : 'eliminada'}, no podrás recuperar los datos asociados. `}
