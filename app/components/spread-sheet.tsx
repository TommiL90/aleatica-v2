import React, { MutableRefObject, useEffect, useRef, useState, useId, useMemo, MouseEventHandler, useCallback } from "react";
import {
  ReactGrid,
  Column,
  Row,
...
interface Props {
  ...
  items: any[];
  rows: Row<DefaultCellTypes | FlagCell | ButtonCell | DateCell | DropdownCell | MaskFieldCell>[];
  columns: Column[];
  emptyElement: any;
  loading: boolean;
...
export default function SpreadSheet({columns, emptyElement, items, loading,  onCellClick, onChange, onChangeColumns, onChangeRows,  onShowRow, onUpdateRow, rows, }: Props) {
  const [columnStickyLeft, setColumsStickyLeft] = useState(0);
  const [_document, set_document] = useState<Document>()
  const [modalCadenamientoInicial, setModalCadenamientoInicial] = useState(false)
  ...
  const handleColumnsReorder = (targetColumnId: Id, columnIds: Id[]) => {
    ...
    const to = reorderedColumns.findIndex((column) => column.columnId === targetColumnId);
    const columnIdxs = columnIds.map((columnId) => reorderedColumns.findIndex((c) => c.columnId === columnId));
      ...
      menuOptions = [
        ...
        },
        {
          ...
          handler: () => {
            ...
            const newRows = items.filter((item, idx) => selectedRowIds.includes(item.idauto));
            const index = Number(selectedRowIds[0]);
        ...
        },
        {
          ...
          handler: () => {
            onChangeRows(
              [...items.filter((item, idx) => !selectedRowIds.includes(item.idauto))].map((item, idx) => ({ ...item, idauto: idx + 1 }))
            )
          }
        },
      ];
    ...
    if (selectionMode === "column") {
      menuOptions = [
        //...menuOptions,
        {
          ...
          label: columnStickyLeft > 0 ? "Desbloquear columna a la izquierda" : "Bloquear columna a la izquierda",
          handler: () => {

            if (columnStickyLeft == 0) {
              const columnIndex = columns.findIndex((item: Column, idx: number) => item.columnId == selectedColIds[0]) + 1;
  ...
  const handleFocusCell = (location: CellLocation) => {
    if (typeof location.rowId === 'number' && location.rowId > 0 && location.columnId === 'descripcionUnidadSimple') {
